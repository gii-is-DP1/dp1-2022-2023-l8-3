...
//		//User user = new User("testUser1", "testUser1");
//		
//		// when(this.userService.findUser(authentication.getName()).get()).thenReturn(user);
//
...
//		config();
//		mockMvc.perform(post("/jugadores/{jugadorId}/edit", TEST_PLAYER_ID).with(csrf()).param("user.username", "laura")
//				.param("firstName", "Laura").param("lastName", "Ruibal")).andExpect(status().is3xxRedirection())
...
//	@WithMockUser(username = "player1", authorities = { "jugador" })
//	@Test
//	void testShowMatchesOfAPlayer() throws Exception {
//		config();
//		Match match = new Match(false, playerService.findJugadorById(TEST_PLAYER_ID));
...
//		this.mockMvc.perform(get("/jugadores/{jugadorId}/playerMatches", TEST_PLAYER_ID_2)).andExpect(status().isOk())
...
//	@Test
//	void testShowFriendsOfAPlayer() throws Exception {
//		this.mockMvc.perform(get("/jugadores/{jugadorId}/playerFriends", TEST_PLAYER_ID)).andExpect(status().isOk())
//				.andExpect(model().attributeExists("jugadorId")).andExpect(model().attributeExists("playerFriends"))
//				.andExpect(view().name("/jugadores/playerFriends"));
//	}
