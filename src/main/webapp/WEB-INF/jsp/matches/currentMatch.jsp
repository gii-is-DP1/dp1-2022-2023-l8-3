			...
			<form:form class="tablero" modelAttribute="match" onsubmit="return validate()">
				...
				<c:if test="${idLoggedPlayer==match.jugador1.id || idLoggedPlayer==match.jugador2.id}">
					<div class="botones">
						...
						<c:choose>
								<c:when test="${match.getMatchTime()>=120}">
									<a class="button" href="<c:url value="/matches/${match.id}/abandonedMatch" />">Abandonar partida </a>
								</c:when>
								<c:otherwise>
									<a class="button" href="<c:url value="#"/>" onclick="alert('Debes esperar 2 minutos para abandonar te quedan ${120-match.getMatchTime()} segundos');">Abandonar partida </a>
								</c:otherwise>
						</c:choose>
